...
llm = create_llm(is_chat_model=True, model="gpt-4", temperature=1)
...
# 定义助手的角色。在此设定中，助手被视为一名花店营销专员，
# 主要职责是为花店老板（即用户）提供关于营销活动的建议和策略
assistant_role_name = "花店营销专员"

# 定义用户的角色。用户在这里是花店的老板，
# 他们可能会向营销专员（即助手）提出关于花店推广活动的需求或询问，然后由营销专员来答复和提供建议。
user_role_name = "花店老板"
...
# 任务说明模板
task_specifier_template = create_chat_prompt_template_from_template(
    human_template=task_specifier_prompt
)
...
# 定义系统初始消息模板，并创建CAMELAgent实例进行交互
...
def get_sys_msgs(assistant_role_name: str, user_role_name: str, task: str):
    ...
    assistant_sys_template = create_chat_prompt_template_from_template(
        system_template=assistant_inception_prompt
    )
    ...
    user_sys_template = create_chat_prompt_template_from_template(
        system_template=user_inception_prompt
    )
